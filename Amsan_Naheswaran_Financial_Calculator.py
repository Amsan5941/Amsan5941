...
##Create a financial calcutor that will help user know there monthly budget and help them save some of there monthly income.In order to calculate the users monthly budget you will need to know there monthly income,
###monthly expense, the percentage they want to save, and how long they want to continue in the saving/budgeting calcutor. The calculator will have two modes; mode 1 called "C" where all the information from the user is
####constant and doesnt change and mode "D" where the information changes every month. The amount the user saves everymonth can be calculated by finding the difference between their income and expense than multiplying by the
#####percentage they want to save. If the users expenses are more than their income than the calcuator will tell the user that "the expenses are greater than the income". The user must input time must also be more or equal to 1.
######The output file must have the total amount the user has reamining, the total the user spent on expenses, and the total they have saved. Make sure the user can veiw the output while running the code so they can use the
#######output file to keep track (if the expenses are greater than income than do not write that in the output file).
...
def Monthly_Calc (type,income,expenses,percent,time): #Function that will be used to calculate the users monthly budget
  for i in range (0,time): #A for loop that will be used to reiterate the amount of months (time) the user wants the calculator to run for 
    if type=="C": #If statment that will be used to determine if the user wants to run mode "C"
      ...
      if left_over<=0: #If statment that will be used to determine if the user has more expenses than income
        ...
        total_saved=((income-expenses)*percent)*time #Calculates the total amount the user saved depending on how long the user wants to continue the program
  ...
  import sys #Function that allows us to write the output in the output file
  sys.stdout=open("C:\\Users\\Amsan\\Documents\\output.txt","a") #Opens the output file so that it can append the information we want to the file
  
  print("The total left over money after {} months is ${}".format(time,left_over_total)) #Prints the total left over money the user has to the output file
  print("The total of expenses after {} months is ${}".format(time,total_expenses)) #Prints the total expenses the amount of moeny user spends to the output file
  print("The total money saved after {} months is ${}".format(time,total_saved)) #Prints the total amount of money the user will save to the output file 
